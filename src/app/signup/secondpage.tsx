...
export default function SecondPage({ mail }: { mail: string }) {
  const user = async (email: string, password: string): Promise<void> => {
    try {
      const addUser = await fetch("http://localhost:4000/Authentication", {
        method: "POST",
        headers: {
  ...
  const onNext = () => router.push("/login");
